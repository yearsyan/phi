...
library 可显式安装 `ToolPermissionApprover`。安装后，名称策略允许、但 effect 超出当前
capability 的工具仍可出现在 Provider 请求中；真正执行前，Agent 会先持久化完整的
tool-call/unknown-result journal，再请求 host 决定“仅允许本次”“按服务端候选规则在本
session 记住”或“拒绝”。library 默认不安装 approver，因此嵌入方不配置时仍 fail closed。
显式的工具名 deny 不能被审批覆盖，内置文件工具的 canonical workspace 校验也不会因
审批放宽。记住的规则随原 session 持久化，但 fork 不继承；规则落盘失败时工具不会执行。

内置 Bash 的记忆规则兼容精确命令、旧式 `prefix:*` 和锚定的 `*` 通配符；`\*` 表示
字面量星号，末尾 ` *` 同时覆盖无参数命令。daemon 只会建议形式稳定的双词前缀或
完整精确命令。含 `&&`、管道、重定向、换行、反引号或 `$()` 的复合命令永远不会命中
前缀/通配规则，只能在完整命令显示后按精确规则审批；常见 shell、wrapper 和脚本解释器也只
建议精确规则。
...
多选、工具权限审批和多 turn 工具轨迹都会实时投影。同一次 assistant 响应包含多个工具调用时，前端会
...
- `flutter`：Android、iOS、macOS、Windows、HarmonyOS/OpenHarmony Flutter daemon 客户端
