...
- `flutter/`：面向 Android、macOS 与 HarmonyOS/OpenHarmony 的 Flutter 客户端；Dart package 名为 `phi_client`。
...
- `flutter/lib/platform/` 放平台适配，`android/`、`macos/`、`ohos/` 只保存各平台 runner、权限和构建配置。平台特例不得污染 daemon wire model 或通用 UI 状态。
...
- Flutter 不提交 `.dart_tool/`、`.flutter-plugins-dependencies`、`build/`、`.gradle/`、`.idea/`、`local.properties`、`oh_modules/`、`node_modules/`、自动签名 profile、keystore、证书路径或生成的 Flutter assets/native libraries。
