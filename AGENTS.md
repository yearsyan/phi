...
- 根 crate `phi`：可嵌入的 agent library，负责模型抽象、agent/tool loop、消息与事件、hooks、上下文压缩、session transcript、skills、MCP、Plan Mode 和 subagent runtime。
...
当前不是完整的 coding-agent CLI。不要假设已经具备 TUI、TLS/origin 校验、多租户、分布式 actor、durable event replay、工具副作用事务回滚或 partial/micro compaction。
...
- `src/tool.rs`、`src/tool/`：工具接口、能力/副作用声明、并发策略和内置工具。Plan Mode 的能力限制必须在运行时再次校验，不能只依赖发给模型的 tool list。
...
- `src/plan.rs`：独立的版本化计划文件、锁和 CAS revision。
...
- `runtime/ask_user.rs`、`runtime/plan_approval.rs`：跨 WebSocket 重连仍由 actor 持有的挂起交互。
...
- daemon attach WS 的主动压缩必须先进入 actor 串行状态；忙时拒绝。实际摘要 prompt、完成 replacement patch 或失败事件必须广播给调用客户端和其他 attach 客户端。
...
- command accepted/rejected 是来源 socket 的 direct response；run、Agent、compaction、ask/approval 和状态事件按 sequence 广播给所有 attach。修改 DTO 时保持这个区分。
- 重连依赖最新 snapshot 和后续 sequence，不提供 durable 历史事件 replay。发生广播 lag 时发送完整 resync snapshot，不补造旧事件。
- `askuser` 和 Plan Exit approval 属于 session actor，不属于某条 WebSocket；断线不能取消它们。审批必须绑定精确 plan revision。
...
- session ID、plan path 和文件名必须通过现有校验/编码，防止路径穿越、大小写别名和超长 path component。
- plan 写入保持 lock + revision CAS + 原子替换；审批不能脱离被批准的 revision。
- `provider.json` 包含明文 API key，Unix 创建权限保持 `0600`。长期 daemon key 只从 key 文件读取，不放入 URL 或普通明文配置环境变量。
- daemon 默认只监听 loopback。项目本身没有 TLS、origin 校验或租户隔离；绑定非本机地址时文档必须明确要求可信前置代理。

## 默认启用与显式启用

- library 保持可嵌入和最小默认面：内置文件/shell 工具、skills、MCP、subagent 工具和 context compactor 都需要调用方显式安装或配置。
- daemon factory 当前显式安装 `askuser`、Plan 工具、默认 compactor，并按 daemon 配置启用 skills 与父 subagent 工具。不要把 daemon 默认行为误写成 library 默认行为。
...
- 不要通过增加并行度绕过 actor、tool effect 或 Plan Mode 约束。并行工具仍需服从 `ToolConcurrency`、副作用分类、超时和 protocol journal。
